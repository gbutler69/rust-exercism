...
mod io {
    ...
    fn reader_munges(key: &str, input: &str, expect: &[u8]) {
        ...
        let mut reader = Xorcism::new(key).reader(&mut input_bytes);
    ...
    fn reader_roundtrip(key: &str, input: &str, expect: &[u8]) {
        ...
        let mut input_bytes = input.as_bytes();
        let mut reader1 = xs.clone().reader(&mut input_bytes);
        let mut reader2 = xs.clone().reader(&mut reader1);
        let mut buf = Vec::with_capacity(input.len());
    ...
    fn writer_roundtrip(key: &str, input: &str, expect: &[u8]) {
        ...
        let xs = Xorcism::new(key);
        {
            let mut writer1 = xs.clone().writer(&mut writer_dest);
            let mut writer2 = xs.writer(&mut writer1);
